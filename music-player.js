export function initMusicPlayer(win, showNotification, specificTrack = null) {
  ...
  function playSong() {
    if (songs.length === 0 || currentSongIndex < 0 || currentSongIndex >= songs.length) {
        showNotification("No songs available or selected.");
        songInfo.textContent = "Playlist empty";
        return;
    }
    const song = songs[currentSongIndex];
    
    // Avoid reloading if the same song source is already set and playing/paused
    let shouldPlay = true;
    if (audio.src !== song.src) {
        audio.src = song.src;
    } else if (!audio.paused) {
        // If already playing the correct song, just update UI and return
        updateSongInfo();
        return; 
    }
    ...
    audio.play().then(() => {
        updateSongInfo();
        // Only show notification if it wasn't already playing this song
        ...
        updateSongInfo(); // Update UI even on error
    });
  }
  ...
  playBtn.addEventListener('click', () => {
    playSong(); // Re-use playSong logic which handles source setting and play/resume
  });
