...
export function createWindow(title, customCode) {
  ...
  } else if (title !== "BSOD") { // BSOD will set its own position/size
    win.style.left = `${50 + Math.random() * 100}px`;
    win.style.top = `${50 + Math.random() * 100}px`;
  }
  win.style.zIndex = getNextZIndex();

  // Set default size if not provided (and not BSOD)
  if (!win.style.width && title !== "BSOD") win.style.width = "400px";
  if (!win.style.height && title !== "BSOD") win.style.height = "300px";
  ...
  switch (title) {
    ...
    case 'Open With': 
      ...
      break;
    case 'BSOD':
      bodyHTML = `<div class="window-body bsod-window">`; // Use a specific class
      content = ``; // Content will be set by initBSOD
      break;
    default:
      bodyHTML = `<div class="window-content">`;
  ...
  // Only create taskbar button if it's not BSOD
  if (title !== "BSOD") {
  ...
  // Make the title bar draggable (only if it exists).
  ...
  // Add resize handle for specific windows (excluding BSOD)
  if (!customCode && title !== "BSOD" && (title === 'Notepad' || title === 'File Explorer' || title === 'Internet Explorer' || title === 'Command Prompt' || title === "VirtualBox")) {
  ...
  setTimeout(() => {
    ...
    if (title === 'BSOD') window.initBSOD(win, window.showNotification); // Initialize BSOD
  }, 0);
